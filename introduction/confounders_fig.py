...
# ############# Ground truth ################
edge_list_1 = [('y0', 'y1', {'w': 'A1'}),
               ('y1', 'y2', {'w': 'B1'}),
               ('y0', 'y2', {'w': 'C1'})]

edge_list_2 = [('y1', 'y0', {'w': 'A2'}),
               ('y2', 'y1', {'w': 'B2'}),
               ('y2', 'y0', {'w': 'C2'})]
...
nx.draw_networkx_labels(G, pos, ax=axes[0], labels=latex_labels)

axes[0].set_title("Common input", fontsize=12, fontweight="bold")

nx.draw_networkx_edges(G, pos, ax=axes[1], edgelist=edge_list_1,
                       connectionstyle=f'arc3, rad = {arc_rad}',
                       alpha=[0, 0, 0], arrowsize=25)
nx.draw_networkx_edges(G, pos, ax=axes[1], edgelist=edge_list_2,
                       connectionstyle=f'arc3, rad = {arc_rad}',
                       alpha=[1, 1, 0], arrowsize=25)
...
nx.draw_networkx_labels(G, pos, ax=axes[1], labels=latex_labels)

axes[1].set_title("Indirect connections", fontsize=12, fontweight="bold")

nx.draw_networkx_edges(G, pos, ax=axes[2], edgelist=edge_list_1,
                       connectionstyle=f'arc3, rad = {arc_rad}',
                       alpha=[0, 0, 0], arrowsize=25)
nx.draw_networkx_edges(G, pos, ax=axes[2], edgelist=edge_list_2,
                       connectionstyle=f'arc3, rad = {arc_rad}',
                       alpha=[0, 0, 1], arrowsize=25)
nx.draw_networkx_edge_labels(G, pos, ax=axes[2],
                             edge_labels={('y1', 'y0'): 0,
                                          ('y2', 'y1'): 0,
                                          ('y2', 'y0'): 1},
                             font_color='red')
nx.draw_networkx_nodes(G, pos, ax=axes[2], node_color='orange', alpha=1.0)
nx.draw_networkx_labels(G, pos, ax=axes[2], labels=latex_labels)
